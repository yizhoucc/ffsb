...
class TD3_ff(TD3):
    ...
    def _train_step(self, step, writer, learning_rate, update_policy):
        ...
        feed_dict = {
            ...
            self.rewards_ph: batch_rewards.reshape(self.batch_size, -1), # reward per time point
    ...
    def learn(self, total_timesteps, callback=None,
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for step in range(total_timesteps):
                ...
                if writer is not None:
                    # Write reward per 'episode'(time point) to tensorboard
                    ep_reward = np.array([reward_]).reshape((1, -1))
                    ep_done = np.array([done]).reshape((1, -1))
                    # print(self.episode_reward, ep_reward,
                ...
                episode_rewards[-1] += reward_ # the current one point reward, not the sum reward
                ...
                if done:
                    ...
                    if maybe_is_success is not None:
                        ...
                        if reward_>5:
                            episode_successes.append(float(1))
                        else:
                            episode_successes.append(float(0))
