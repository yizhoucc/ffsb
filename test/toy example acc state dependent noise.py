...
for anoise in (1-np.power(1e-3,np.linspace(0,1,10))):
    ...
    quickleg(ax,bbox_to_anchor=(0,0))
    quickspine(ax)
    plt.title('anoise level = {}'.format(anoise))
    plt.show()
...
studies suggest that two optic flows are not seperable by perception.
so, seems like integration happends first.
we can:
integrate two optic flows, to have the v_obs. we naturally have the mean and variance from the vp and vo and their corresponding noise per dt.
then, we update using kalman filter. here we have two priors, from the vp prediction and vo prediction. we have one likelihood, that is the v-obs. we integrate them together.
