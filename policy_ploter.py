...
def plot_policy_surface(belief,torch_model,name_index):
    ...
    plt.imshow(policy_data_v,origin='lower',extent=[r_labels[0],r_labels[-1],a_labels[0],a_labels[-1]])
...
def plot_policy_surfaces(belief,torch_model1,torch_model2,name_index):
    ...
    for ri in range(len(r_labels)):
        for ai in range(len(a_labels)):
            belief[0]=r_labels[ri]
            belief[1]=a_labels[ai]
            policy1_data_v[ri,ai]=torch_model1(belief)[0]
    ...
    ax[0,1].imshow(policy2_data_v,origin='lower',extent=[a_labels[0],a_labels[-1],r_labels[0],r_labels[-1]])


    ax[1,0].set_title('selu angular velocity',fontsize=24)
    ax[1,0].imshow(policy1_data_w,origin='lower',extent=[a_labels[0],a_labels[-1],r_labels[0],r_labels[-1]])
    ax[1,1].set_title('relu angular velocity',fontsize=24)
    ax[1,1].imshow(policy2_data_w,origin='lower',extent=[a_labels[0],a_labels[-1],r_labels[0],r_labels[-1]])
