...
def pertexample():
    with initiate_plot(3, 3, 300) as fig:
        ...
        ax.plot([i*200 for i in df.iloc[0].action_v], '--',
                color=color_settings['v'], label='v control')
        ax.plot(df.iloc[0].pos_w, color=color_settings['w'], label='real w')
        ax.plot([i*200 for i in df.iloc[0].action_w], '--',
                color=color_settings['w'], label='w control')
        ax.legend()
    ...
    with initiate_plot(8, 2, 300) as fig:
        for i, density in enumerate(densities):
            ...
            for xx, yy in zip(x, y):
                plt.plot(xx, yy, markersize=3, marker=next(
...
def plot_belief(env, title='title', alpha_factor=1, **kwargs):
    ...
    # yellow goal
    plot_circle(np.eye(2)*env.phi[-1, 0].item(),
                [env.goalx, env.goaly], ax=ax, color='y')
...
def compute_H_monkey(env,
    ...
    grads = torch.autograd.grad(
        loss, theta_estimation, create_graph=True, allow_unused=True)[0]
    H = torch.zeros(H_dim, H_dim)
    for i in range(H_dim):
        print(i)
        H[i] = torch.autograd.grad(
            grads[i], theta_estimation, retain_graph=True, allow_unused=True)[0].view(-1)
    return H


def compute_H(env, agent, theta_estimation, true_theta, phi, trajectory_data=None, H_dim=7,
              num_episodes=100, is1d=False):
    states, actions, tasks = trajectory(agent, torch.Tensor(
        phi), torch.Tensor(true_theta), env, num_episodes, is1d=is1d)
    theta_estimation = torch.nn.Parameter(torch.Tensor(theta_estimation))
    phi = torch.nn.Parameter(torch.Tensor(phi))
    phi.requires_grad = False
    loss = getLoss(agent, actions, tasks, phi, theta_estimation,
                   env, states=states, gpu=False)
    grads = torch.autograd.grad(
        loss, theta_estimation, create_graph=True, allow_unused=True)[0]
    H = torch.zeros(H_dim, H_dim)
    for i in range(H_dim):
...
def compute_loss(env, agent, theta_estimation, true_theta, phi,
    ...
    theta_estimation = torch.nn.Parameter(torch.Tensor(theta_estimation))
    loss = getLoss(agent, actions, tasks, torch.Tensor(
        phi), theta_estimation, env, states=states)
    return loss


def compute_loss_wrapped(env, agent, true_theta, phi, trajectory_data=None,
                         num_episodes=100, is1d=False):
    def new_function(theta_estimation): return compute_loss(
        env, agent, theta_estimation, true_theta, phi, num_episodes=100, is1d=is1d)
    return new_function
...
def plot_background(ax, xyrange, mu, evectors, loss_function,
    ...
    X, Y = np.meshgrid(np.linspace(xyrange[0][0], xyrange[0][1], number_pixels), np.linspace(
        xyrange[1][0], xyrange[1][1], number_pixels))
    if method == 'PCA':
        for i, u in enumerate(np.linspace(xyrange[1][0], xyrange[1][1], number_pixels)):
            for j, v in enumerate(np.linspace(xyrange[0][0], xyrange[0][1], number_pixels)):
                ...
                if not np.array_equal(reconstructed_theta, reconstructed_theta.clip(1e-4, 999)):
                    ...
                    print(loss_function(torch.Tensor(
                        reconstructed_theta).reshape(-1, 1)))
                    background_data[i, j] = loss_function(
                        torch.Tensor(reconstructed_theta).reshape(-1, 1))
        # elif method=='v_noise':
    # elif method =='random':
    #   for i,u in enumerate(np.linspace(xyrange[1][0],xyrange[1][1],number_pixels)):
    #     for j,v in enumerate(np.linspace(xyrange[0][0],xyrange[0][1],number_pixels)):
...
def loss_surface_two_param(mu, loss_function, param_pair,
    ...
    if type(mu) == torch.Tensor:
        mu = np.array(mu)
    background_data = np.zeros((number_pixels, number_pixels))
    X, Y = np.meshgrid(np.linspace(param_range[0][0], param_range[0][1], number_pixels), np.linspace(
        param_range[1][0], param_range[1][1], number_pixels))
...
def plot_inverse_trajectorys(theta_trajectorys, true_theta,
    ...
    ax.set_title('Inverse for theta estimation in a PCs space. Sample corr: {:.1f}'.format(
        np.corrcoef(score[:, 0], score[:, 1])[0, 1]))
    ...
    # plot log likelihood contour
    if background_contour:
        print(np.array(true_theta).reshape(-1, 1),
              np.array(phi).reshape(-1, 1))
        loss_function = compute_loss_wrapped(env, agent, np.array(true_theta).reshape(
            -1, 1), np.array(phi).reshape(-1, 1), trajectory_data=None, num_episodes=100)
        current_xrange = ax.get_xlim()
        current_yrange = ax.get_ylim()
        xyrange = [current_xrange, current_yrange]
        # ax1.contourf(X,Y,background_data)
        background_data = plot_background(ax, xyrange, mu, evectors, loss_function,
                                          number_pixels=number_pixels, alpha=alpha) if background_data is None else background_data
...
def diagnose_trial(index, phi, eaction, etask, theta, agent, env, num_trials=10,
    ...
    with torch.no_grad():
        for trial_i in range(num_trials):
            env.reset(phi=phi, theta=theta, goal_position=etask,
                      vctrl=initv, wctrl=initw)
            epbliefs = []
            epbcov = []
            epactions = []
            epstates = []
            ...
            while t < len(eaction):
                ...
                if estate is None and guided:
                    ...
                    _, done = env(torch.tensor(eaction[t]).reshape(
                        1, -1), task_param=theta)
                    ...
                    if t+1 < (estate.shape[1]):
                        ...
                        _, done = env(torch.tensor(eaction[t]).reshape(
                            1, -1), task_param=theta, state=estate[:, t+1].view(-1, 1))
        ...
        # used when agent on its own
        astate = torch.stack(agent_states)[0, :, :].t()
        if estate is None:
            estate = astate
        return_dict = {
...
def diagnose_plot(estate, theta, eaction, etask, agent_actions, agent_beliefs,
    ...
    ax1.plot(torch.tensor(eaction)[:, 0],
             label='monkey', alpha=0.4, linewidth=10)
    ax1.legend()
    ...
    ax2.plot(torch.tensor(eaction)[:, 1],
             label='monkey', alpha=0.4, linewidth=10)
    ax2.legend()
    ...
    goalcircle = plt.Circle(
        (etask[0], etask[1]), theta[6, 0], color='y', alpha=0.5)
    ...
    for i in range(len(agent_beliefs)):
        for t in range(len(agent_beliefs[i][:, :, 0])):
            ...
            plot_cov_ellipse(cov, pos, nstd=2, color=None,
                             ax=ax4, alpha_factor=0.2)
...
def diagnose_plot_theta(agent, env, phi, theta_init, theta_final, nplots,):
    def sample_trials(agent, env, theta, phi, etask, num_trials=5):
        ...
        with torch.no_grad():
            for trial_i in range(num_trials):
                env.reset(phi=phi, theta=theta, goal_position=etask[0], vctrl=torch.zeros(
                    1), wctrl=torch.zeros(1))
                epbliefs = []
                epbcov = []
                epactions = []
                epstates = []
    ...
    for n in range(nplots):
        ...
        goalcircle = plt.Circle(
            (etask[0][0], etask[0][1]), 0.13, color='y', alpha=0.5)
...
def diagnose_plot_theta1d(agent, env, phi, theta_init, theta_final, nplots, initv=0.):
        ...
        with torch.no_grad():
            for trial_i in range(num_trials):
                env.reset(phi=phi, theta=theta,
                          goal_position=etask[0]*torch.ones(1), initv=initv*torch.ones(1))
                epbliefs = []
                epbcov = []
                epactions = []
                epstates = []
    ...
    for n in range(nplots):
        ...
        ax1.set_ylabel('world y, cm')
        ax1.set_title('state plot')
        data = sample_trials(agent, env, theta, phi, etask,
                             num_trials=1, initv=initv)
        estate = data['estate']
        # if n==1:
        #   print(agent_beliefs[0][:,:,0][:,0])
        ax1.plot(list(range(len(estate[0, :]))),
                 estate[0, :], color='r', alpha=0.5)
...
def diagnose_stopcompare(estate, theta, eaction, etask, agent_actions, agent_beliefs, agent_covs,
    ...
    for i in indls:
        goalcircle = plt.Circle(
            (tasks[i][0][0], tasks[i][0][1]), 0.13, color='y', alpha=0.3, fill=False)
        ax1.add_patch(goalcircle)
        ax1.plot(states[i][0, :], states[i][1, :], 'steelblue', alpha=0.2)
        ax1.scatter(states[i][0, -1], states[i][1, -1],
...
def diagnose_plot_xaqgrantoverhead(estate, theta, eaction, etask, agent_actions, agent_beliefs,
    ...
    ax3.plot(estate[0, :], estate[1, :], alpha=0.4,
             linewidth=2, label='actual positions')
    goalcircle = plt.Circle(
        (etask[0][0], etask[0][1]), 0.13, color='y', alpha=0.4)
    ax3.add_patch(goalcircle)
    ax3.set_xlim([-0.1, 1.1])
    ax3.set_ylim([-0.6, 0.6])
...
def diagnose_plot_stop(estate, theta, eaction, etask, agent_actions, agent_beliefs,
    ...
    ax4.set_title('belief')
    goalcircle = plt.Circle(
        (etask[0][0], etask[0][1]), theta[6, 0], color='y', alpha=0.5)
    ax4.add_patch(goalcircle)
    ax4.set_xlim([-0.1, 1.1])
    ax4.set_ylim([-0.6, 0.6])
    for i in range(len(agent_actions)):
...
def diagnose_dynamic_wrapped():
    def diagnose_dynamic(index, phi, eaction, etask, theta, agent, env, num_trials=10, monkeystate=None, estate=None, guided=True, initv=0):
        ...
        with torch.no_grad():
            for trial_i in range(num_trials):
                env.reset(phi=phi, theta=theta,
                          goal_position=etask[0], initv=initv)
                epbliefs = []
                epbcov = []
                epactions = []
                epstates = []
    ...
    for a in diaginfo['astate']:
        ...
        plt.title(((diaginfo['estate'][0, -1]-a[-1, 0]) **
                  2+(a[-1, 1]-diaginfo['estate'][1, -1])**2)**0.5)
...
def agent_double_gain():
    ...
    for trialind in range(res1['num_trials']):
        err = torch.norm(torch.tensor(
            res1['task'][trialind])-res1['agent_states'][trialind][-1][:2])
        errs1.append(err)
    ...
    for trialind in range(res2['num_trials']):
        err = torch.norm(torch.tensor(
            res2['task'][trialind])-res2['agent_states'][trialind][-1][:2])
        errs2.append(err)
...
def overhead_skip_density(input):
    ...
    with initiate_plot(1.8, 1.8, 300) as fig:
        ...
        for trial_i in range(input['num_trials']):
            ...
            dx = input['agent_states'][trial_i][-1, 1] * \
                400-input['task'][trial_i][1]*400
            dy = input['agent_states'][trial_i][-1, 0] * \
                400-input['task'][trial_i][0]*400
            d2goal = (dx**2+dy**2)**0.5
            d2start = (input['agent_states'][trial_i][-1, 1] **
                       2+input['agent_states'][trial_i][-1, 0]**2)**0.5*400
...
def diagnose_plot_theta(agent, env, phi, theta_init, theta_final, nplots, etask=[[0.7, -0.3]], initv=0., initw=0., mkactions=None):
    ...
    for n in range(nplots):
        ...
        # ax1.set_title('state plot')
        data = sample_trials(agent, env, theta, phi, etask,
                             num_trials=1, initv=initv, initw=initw)
        estate = data['estate']
        ax1.plot(estate[0, :]*200, estate[1, :]*200, color='r', alpha=0.5)
        goalcircle = plt.Circle(
            (etask[0]*200, etask[1]*200), 65, color='y', alpha=0.5)
        ax1.add_patch(goalcircle)
    ...
    # v and w
        ...
        for i in range(len(agent_actions)):
            ax2.plot(
                np.arange(len(agent_actions[i]))/10, agent_actions[i], alpha=0.7)
        ax2.set_ylim([-1.1, 1.1])
...
def vary_theta_new(agent, env, phi, theta_init, theta_final, nplots, etask=[0.7, -0.3], initv=0., initw=0., mkactions=None, pert=None, ntrials=10, plotallb=False, gradientpath=True, gradientreso=10, savename=None):
    ...
    def sample_trials(agent, env, theta, phi, thistask, initv=0., initw=0., action_noise=0.1, pert=None,):
        ...
        agent_states = []
        with torch.no_grad():
            env.reset(phi=phi, theta=theta, goal_position=thistask,
                      vctrl=initv, wctrl=initw)
            if pert is None:
                env.obs_traj = np.vstack([np.array([0, 0]), np.random.normal(0, 1, size=(
                    env.episode_len, 2))*np.asfarray(theta[4:6]).T, np.zeros(shape=(20, 2))])
            else:
                env.obs_traj = np.vstack([np.array([0, 0]), np.random.normal(
                    0, 1, size=(pert))*np.asfarray(theta[4:6]).T + pert, np.zeros(shape=(20, 2))])
            epbliefs = []
            epbcov = []
            epactions = []
            epstates = []
    ...
    with initiate_plot(3*nplots, 3, 300) as fig:
        ...
        for n in range(nplots):
            ...
            for i in range(ntrials):
                with suppress():
                    data = sample_trials(
                        agent, env, theta, phi, etask, initv=initv, initw=initw, pert=pert)
                estate = data['estate']
                agent_beliefs = data['agent_beliefs']
                ...
                ax = subplotmap[n]
                if not gradientpath:
                    ax.plot(estate[0, :]*200, estate[1, :]*200,
                            color=color_settings['s'], alpha=0.5)
                else:
                    for t in range(estate.shape[1]-1):
                        colorbinsize = 1/gradientreso
                        colormap = colorlinspace(
                            [0, 0, 1], [0.1, 1, 0.1], gradientreso)
                        c = colormap[int(
                            abs(agent_actions[t, 1].item())/colorbinsize)]
                        ax.plot([estate[0, t]*200, estate[0, t+1]*200], [estate[1, t]
                                * 200, estate[1, t+1]*200], c=c, alpha=1, linewidth=0.5)
                ax.scatter(estate[0, -1]*200, estate[1, -1]*200,
                           edgecolor='none', color=color_settings['s'], alpha=1)
                if i == 0:
                    goalcircle = plt.Circle(
                        (etask[0]*200, etask[1]*200), 65, edgecolor='k', facecolor='none', alpha=1, linewidth=5)
                ...
                if plotallb:
                    for t in range(len(agent_beliefs[0][:, :, 0])):
                        ...
                        pos = [agent_beliefs[0][:, :, 0][t, 0]*200,
                               agent_beliefs[0][:, :, 0][t, 1]*200]
                        plot_cov_ellipse(
                            cov, pos, nstd=3, color=color_settings['b'], ax=ax, alpha=0.05)
                else:
                    ...
                    pos = [agent_beliefs[0][:, :, 0][t, 0]*200,
                           agent_beliefs[0][:, :, 0][t, 1]*200]
                    plot_cov_ellipse(
                        cov, pos, nstd=3, edgecolor=color_settings['b'], color='none', ax=ax, alpha=1, linewidth=0.5)
    ...
    with initiate_plot(3*nplots, 3*3, 300) as fig:
        ...
        for n in range(nplots):
            ...
            for i in range(ntrials):
                ...
                ax.plot(estate[0, :]*200, estate[1, :]*200,
                        color=color_settings['s'], alpha=0.5)
                ax.scatter(estate[0, -1]*200, estate[1, -1]
                           * 200, color=color_settings['a'], alpha=0.5)
                if i == 0:
                    goalcircle = plt.Circle(
                        (etask[0]*200, etask[1]*200), 65, color=color_settings['goal'], edgecolor='none', alpha=0.3, linewidth=0.8)
                    ax.add_patch(goalcircle)
                    ax.set_aspect('equal')
                    ax.set_xlabel('world x [cm]')
                    ax.set_ylabel('world y [cm]')
                ...
                if pert is not None and i == 0:
                    ...
                    pertcolor = matplotlib.colors.ListedColormap(
                        np.linspace([1, 0, 0, 0], [1, 0, 0, np.max(pertv)], 200))
                    pertvbox = np.broadcast_to(pertv, (20, len(pertv)))
                    im = ax.imshow(pertvbox, interpolation='bilinear', cmap=pertcolor,
                                   aspect='auto', origin='lower', extent=[0, len(pertv)/10, -0.2, 1])

                ax.set_ylabel('forward control')
                for j in range(len(agent_actions)):
                    ax.plot(np.arange(len(agent_actions[j]))/10, agent_actions[j]
                            [:, 0], alpha=0.7, color=color_settings['model'], linewidth=1)
                ax.set_ylim([-0.2, 1.1])
                ax.set_yticks([0, 1.])
                # ax.set_xticks([])
                ax.set_xlim(left=0.)
                ...
                if n == nplots//2 and mkactions is not None:
                    ax.plot(np.arange(len(mkactions))/10,
                            mkactions[:, 0], alpha=0.7, color=color_settings['a'])
                ...
                if pert is not None and i == 0:
                    ...
                    pertcolor = matplotlib.colors.ListedColormap(
                        np.linspace([1, 0, 0, 0], [1, 0, 0, np.max(pertw)], 100))
                    pertwbox = np.broadcast_to(pertw, (20, len(pertw)))
                    im = ax.imshow(pertwbox, interpolation='bilinear', cmap=pertcolor,
                                   aspect='auto', origin='lower', extent=[0, len(pertw)/10, -1, 1])

                ax.set_xlabel('t [s]')
                ax.set_ylabel('angular control')
                for j in range(len(agent_actions)):
                    ax.plot(np.arange(len(
                        agent_actions[j]))/10, agent_actions[j][:, 1], alpha=0.7, color=color_settings['model'])
                ax.set_ylim([-1.1, 1.1])
                ax.set_yticks([-1., 0, 1.])
                ax.set_xlim(left=0.)
                ax.xaxis.set_major_locator(MaxNLocator(integer=True))
                ...
                if n == nplots//2 and mkactions is not None:
                    ax.plot(np.arange(len(mkactions))/10,
                            mkactions[:, 1], alpha=0.7, color=color_settings['a'])
...
def two_theta_curvaturehist(agent, env, phi, thetas, labels=['ASD model', 'Ctrl model'], etask=[0.5, 0.5], initv=0., initw=0., mkactions=None, pert=None, ntrials=50):
        ...
        with suppress():
            for task in tasks:
                while len(states) < ntrials:
                    ...
                    epactions, _, _, epstates = run_trial(
    ...
    for n, theta in enumerate(thetas):
        ...
        with torch.no_grad():
            curvature = _eval_curvature2(agent, env, phi, theta, [
                                         etask]*50, ntrials=ntrials)
...
def two_theta_overhead(agent, env, phi, thetas, labels=['model1', 'model2'], etask=[0.7, -0.3], initv=0., initw=0., mkactions=None, pert=None, ntrials=10):
    ...
    with initiate_plot(3, 3, 300) as fig:
        ...
        for n, theta in enumerate(thetas):
            ...
            # mean overhead
            ax.plot(meanxy[:, 0]*200, meanxy[:, 1]*200,
                    color=color_settings['pair'][n], alpha=0.5, label=labels[n])
        ax.legend()

        goalcircle = plt.Circle((etask[0]*200, etask[1]*200), 65,
                                color=color_settings['goal'], edgecolor='none', alpha=0.3, linewidth=0.8)
        ax.add_patch(goalcircle)
        ax.set_aspect('equal')
        ax.set_xlabel('world x [cm]')
        ax.set_ylabel('world y [cm]')
...
def vary_theta_ctrl(agent, env, phi, theta_init, theta_final, nplots=5, etask=[[0.7, -0.3]], initv=0., initw=0., mkactions=None, ntrials=10, pert=None):
    ...
    for n in range(nplots):  # this is actually number of theta
        ...
        for i in range(ntrials):
            with suppress():
                data = sample_trials(agent, env, theta, phi,
                                     etask, initv=initv, initw=initw, pert=pert)
        ...
        if n == 2:
            ax.fill_between(
                ts, y1=low, y2=high, color=color_settings['a'], alpha=0.2, edgecolor='none', label=labels[n])
        else:
            ax.fill_between(
                ts, y1=low, y2=high, color=colors[n], alpha=0.2, edgecolor='none', label=labels[n])
    ax.set_ylim([-0.2, 1.1])
    ax.set_yticks([0, 1.])
    ax.set_xlim(left=0.)
    ax.spines['top'].set_visible(False)
...
def likelihoodvstime(agent=None,
    ...
    with torch.no_grad():
        ...
        for t, mk_action in enumerate(mkactionep[1:]):
            ...
            for env in envls:
                env.b, env.P = env.belief_step(env.b, env.P, env.observations(
                    nextstate), torch.tensor(mk_action).view(1, -1))
                previous_action = mk_action  # current action is prev action for next time
                env.trial_timer += 1
                env.decision_info = env.wrap_decision_info(
                    previous_action=previous_action, time=env.trial_timer)

            action_ll = [logll(torch.tensor(mk_action), a, std=np.sqrt(
                action_var)).view(-1).tolist() for a in action]
            obs_ll = [logll(error=env.obs_err(), std=theta[4:6].view(
                1, -1)).view(-1).tolist() for env in envls]
...
def irc_pert(agent, env, phi, theta, etask=[[0.7, -0.3]], initv=0., initw=0., mkactions=None, pert=None, ntrials=10):
    ...
    high = np.percentile(agent_v, 100-percentile, axis=0)
    ts = np.arange(len(agent_v.T))*0.1
    ax.fill_between(ts, y1=low, y2=high,
                    color=color_settings['model'], alpha=0.2, edgecolor='none', label='IRC')
    ax.set_ylim([-0.2, 1.1])
    ax.set_yticks([0, 1.])
    ax.set_xlim(left=0.)
    ax.spines['top'].set_visible(False)
    ...
    # ax.set_xlabel('t [s]')
    ax.plot(np.arange(len(mkactions))/10,
            mkactions[:, 0], alpha=0.7, color=color_settings['a'], label='monkey')
    leg = ax.legend()
    for lh in leg.legendHandles:
        lh.set_alpha(1)
    ...
    pertw = np.abs(pert.T[1])
    pertcolor = matplotlib.colors.ListedColormap(
        np.linspace([1, 0, 0, 0], [1, 0, 0, np.max(pertw)], 200))
    pertwbox = np.broadcast_to(pertw, (20, len(pertw)))
...
def policygiventheta(env=None, agent=None, theta=None):
    ...
    with initiate_plot(15, 10, 300) as fig, warnings.catch_warnings():
        ...
        ax.set_xlim([0, 1])
        ax.set_ylim([-1, 1])
        ax.plot(np.linspace(0, 1, reso),
                actions[:, 0], color=color_settings['v'])
        ax.plot(np.linspace(0, 1, reso),
                actions[:, 1], color=color_settings['w'])
        ax.spines['top'].set_visible(False)
        ax.spines['right'].set_visible(False)
        ...
        orange_patch = mpatches.Patch(
            color=color_settings['w'], label='angular')
        ax.legend(handles=[teal_patch, orange_patch],
...
def single_trial_overhead():
    ...
    with initiate_plot(3, 3, 300) as fig, warnings.catch_warnings():
        ...
        # ax.add_patch(circle)
        goalcircle = plt.Circle(
            (etask[0], etask[1]), 0.13, color=color_settings['goal'], edgecolor='none', linewidth=0., alpha=0.5)
        ax1.add_patch(goalcircle)
        ax1.set_xlim([-0.1, 1.1])
        ax1.set_ylim([-0.6, 0.6])
        ax1.spines['top'].set_visible(False)
        ...
        with torch.no_grad():
            ...
            agent_states = []
            env.reset(
                phi=phi, theta=input['theta'], goal_position=etask, vctrl=initv, wctrl=initw)
            epbliefs = []
            epbcov = []
            epactions = []
            epstates = []
            ...
            for t in range(len(agent_beliefs[0][:, :, 0])-1):
                ...
                pos = [agent_beliefs[0][:, :, 0][t, 0],
                       agent_beliefs[0][:, :, 0][t, 1]]
                plot_cov_ellipse(cov, pos, nstd=2,
                                 color=color_settings['b'], ax=ax1, alpha=0.5)
        # mk state, blue
        ax1.plot(input['mkdata']['states'][0][:, 0],
                 input['mkdata']['states'][0][:, 1], alpha=0.9)
...
def plot_critic_polar(dmin=0.2,
    ...
    with torch.no_grad():
        ...
        start = base.clone().detach()
        start[0] = dmin
        start[1] = -0.75
        xend = start.clone().detach()
        yend = start.clone().detach()
        xend[0] = 1
        yend[1] = 0.75
        ...
        for xi in range(2*nbin):
            ...
            for yi in range(nbin):
                ...
                value = critic(
                    torch.cat([thestate, agent(thestate).flatten()]))
    ...
    with initiate_plot(5, 3, 300) as fig, warnings.catch_warnings():
        warnings.simplefilter('ignore')
        a, d = np.meshgrid(np.linspace(-0.75, 0.75, nbin),
                           np.linspace(dmin, 1, 2*nbin))
        fig = plt.figure()
...
def plot_inverse_trend(inverse_data):
    ...
    with initiate_plot(30, 2, 300) as fig:
        ...
        for n in range(numparams):
            ...
            ax.plot(y)
            ax.set_ylim([min(parameter_range[n][0], min(y)[0]),
                        max(parameter_range[n][1], max(y)[0])])
        plt.show()
    # grad
    with initiate_plot(30, 2, 300) as fig:
...
def compute_H_monkey(env,
    ...
    print('bp for grad')
    grads = torch.autograd.grad(
        loss, thistheta, create_graph=True, allow_unused=True)[0]
    H = torch.zeros(H_dim, H_dim)
    for i in range(H_dim):
        print('calculate {}th row of H'.format(i))
        H[i] = torch.autograd.grad(
            grads[i], thistheta, retain_graph=True, allow_unused=True)[0].view(-1)
    return H
...
def trial_data(input):
    ...
    result['task'] = [input['mkdata']['task'][i]
                      for i in input['mkdata']['trial_index']]
    result['mk_actions'] = [input['mkdata']['actions'][i]
                            for i in input['mkdata']['trial_index']]
    result['mk_states'] = [input['mkdata']['states'][i]
                           for i in input['mkdata']['trial_index']]
    ...
    if 'task' not in input:  # random some task
        for trial_i in range(input['num_trials']):
            ...
            task = [(torch.cos(angle)*distance).item(),
                    (torch.sin(angle)*distance).item()]
        ...
        for trial_i in range(input['num_trials']):
            ...
            if input['use_mk_data']:  # using mk data
                ...
                while t < len(result['mk_actions'][trial_i]):
                    ...
                    epstates.append(env.s)
                    t = t+1
                # the predicted belief of monkey
                result['agent_beliefs'].append(torch.stack(epbliefs))
                # the predicted uncertainty of monkey
                result['agent_covs'].append(epbcov)
                # the actions agent try to excute
                result['agent_actions'].append(torch.stack(epactions))
                result['agent_states'].append(torch.stack(
                    epstates)[:, :, 0])  # will be same as mk states
    ...
    with torch.no_grad():
        ...
        while row_cursor < score.shape[0]-1:
            ...
            ax.plot(score[row_cursor-1:row_cursor+1, 0],
                    ...
                    color='g')
        # plot log likelihood contour
        sample_states, sample_actions, sample_tasks = sample_batch(
            states=states, actions=actions, tasks=tasks, batch_size=loss_sample_size)
        ...
        maxaxis = max(abs(current_xrange[0]-finaltheta[0]), abs(current_xrange[1]-finaltheta[0]), abs(
            current_yrange[0]-finaltheta[1]), abs(current_yrange[1]-finaltheta[1]))
...
def plot_background(
    ...
    with torch.no_grad():
        ...
        if background_data is None:
            ...
            for i, u in enumerate(np.linspace(xyrange[1][0], xyrange[1][1], number_pixels)):
                for j, v in enumerate(np.linspace(xyrange[0][0], xyrange[0][1], number_pixels)):
                    score = np.array([u, v])
                    reconstructed_theta = score@evectors.transpose()[:2, :]+mu
                    reconstructed_theta = torch.tensor(
                        reconstructed_theta).float()
        ...
        if look == 'contour':
            c = ax.contourf(X, Y, background_data.transpose(),
...
def plot_background_par(
    ...
    if background_data is None:
        ...
        jobparam = [(u, v, i, j, background_data) for (i, u), (j, v) in zip(enumerate(np.linspace(
            xyrange[1][0], xyrange[1][1], number_pixels)), enumerate(np.linspace(xyrange[0][0], xyrange[0][1], number_pixels)))]
...
def monkey_loss_wrapped(
        ...
        action_var=0.001,
        **kwargs):
    def new_function(theta_estimation): return monkeyloss(
        agent=agent,
        actions=actions,
        tasks=tasks,
        phi=phi,
...
def plot_background_ev(
    ...
    if background_data is None:
        background_data = np.zeros((number_pixels, number_pixels))
        with torch.no_grad():
            X = np.linspace(theta-ev1*0.5*scale, theta +
                            ev1*0.5*scale, number_pixels)
            Y = np.linspace(theta-ev2*0.5*scale, theta +
                            ev2*0.5*scale, number_pixels)
...
def plotctrl(input, **kwargs):
    ...
    with initiate_plot(3.8, 1.8, 300) as fig, warnings.catch_warnings():
        ...
        # plot data
        for trial_i in range(input['num_trials']):
            ax.plot(input['agent_actions'][trial_i][:, 0], color=color[0],
                    alpha=1/input['num_trials']**.3, label=prefix+labels[0])
            ax.plot(input['agent_actions'][trial_i][:, 1], color=color[1],
                    alpha=1/input['num_trials']**.3, label=prefix+labels[1])
        # legend and label
...
def plotctrl_mk(indls, actions=None, **kwargs):
    ...
    with initiate_plot(3.8, 1.8, 300) as fig, warnings.catch_warnings():
        ...
        for trial_i in indls:
            ax.plot(actions[trial_i][1:, 0], color=color[0],
                    alpha=1/len(indls)**.3, label=prefix+labels[0])
            ax.plot(actions[trial_i][1:, 1], color=color[1],
                    alpha=1/len(indls)**.3, label=prefix+labels[1])
        # legend and label
        handles, labels = ax.get_legend_handles_labels()
        by_label = dict(zip(labels, handles))
        ax.legend(by_label.values(), by_label.keys(), loc='upper right')
...
def plotctrlasd(actionslike, **kwargs):
    ...
    with initiate_plot(3, 4, 300) as fig, warnings.catch_warnings():
        ...
        for trial_i in range(num_trials):
            ax1.plot(actionslike[trial_i][:, 0],
                     color=color[0], label=prefix+labels[0])
            ax2.plot(actionslike[trial_i][:, 1],
                     color=color[1], label=prefix+labels[1])
        # legend and label
        quickleg(ax1)
        quickleg(ax2)
        quickspine(ax1)
...
def plotoverhead(input, alpha=0.8, **kwargs):
    ...
    if 'ax' in kwargs:
        ...
        ax.plot(np.linspace(0, 230 + 7),
                np.tan(np.deg2rad(55)) * np.linspace(0, 230 + 7) - 10, c='k', ls=':')
        goal = plt.Circle(np.array(input['task'][0])@np.array([[0, 1], [-1, 0]])
                          * 400, 65, color=color_settings['goal'], alpha=alpha, label='target')
        ax.add_patch(goal)
        for trial_i in range(input['num_trials']):
            input['agent_states'][trial_i][:, 0] = input['agent_states'][trial_i][:,
                                                                                  0]-input['agent_states'][trial_i][0, 0]
            input['agent_states'][trial_i][:, 1] = input['agent_states'][trial_i][:,
                                                                                  1]-input['agent_states'][trial_i][0, 1]
        ...
        with initiate_plot(1.8, 1.8, 300) as fig:
            ...
            ax.plot(np.linspace(0, 230 + 7),
                    np.tan(np.deg2rad(55)) * np.linspace(0, 230 + 7) - 10, c='k', ls=':')
            goal = plt.Circle(np.array(input['task'][0])@np.array(
                [[0, 1], [-1, 0]])*400, 65, color=color_settings['goal'], alpha=alpha, label='target')
            ax.add_patch(goal)
...
def plotoverheadhuman(indls, states, tasks, alpha=0.8, fontsize=5, ax=None, color=color_settings['s'], label=None):
    if ax:
        for trial_i in indls:
            xs = states[trial_i][:, 0]
            ys = states[trial_i][:, 1]
            ax.plot(-ys*400, xs*400, c=color, lw=1,
                    ls='-', label=label, alpha=alpha)
        return ax
    with initiate_plot(3, 3, 300) as fig:
        ax = fig.add_subplot(111)
        ax.set_aspect('equal')
        ...
        goal = plt.Circle(np.array(tasks[indls[0]])@np.array([[0, 1], [-1, 0]])*400, 65,
                          color=color_settings['goal'], edgecolor='none', alpha=alpha, label='target')
        ax.add_patch(goal)
        for trial_i in indls:
            xs = states[trial_i][:, 0]
            ys = states[trial_i][:, 1]
...
def plotoverheadhuman_compare(data1, data2, alpha=0.8, fontsize=5):
    with initiate_plot(3, 3, 300) as fig:
        ...
        goal = plt.Circle(np.array(data1['task'][0])@np.array([[0, 1], [-1, 0]])*400, 65,
                          color=color_settings['goal'], facecolor='none', alpha=alpha, label='target')
        ax.add_patch(goal)
        for trial_i in range(data1['num_trials']):
            data1['agent_states'][trial_i][:, 0] = data1['agent_states'][trial_i][:,
                                                                                  0]-data1['agent_states'][trial_i][0, 0]
            data1['agent_states'][trial_i][:, 1] = data1['agent_states'][trial_i][:,
                                                                                  1]-data1['agent_states'][trial_i][0, 1]
            ax.plot(-data1['agent_states'][trial_i][:, 1]*400, data1['agent_states']
                    [trial_i][:, 0]*400, c=color_settings['s'], lw=1, ls='-')
            data2['agent_states'][trial_i][:, 0] = data2['agent_states'][trial_i][:,
                                                                                  0]-data2['agent_states'][trial_i][0, 0]
            data2['agent_states'][trial_i][:, 1] = data2['agent_states'][trial_i][:,
                                                                                  1]-data2['agent_states'][trial_i][0, 1]
            ax.plot(-data2['agent_states'][trial_i][:min(data2['agent_states'][trial_i][:, 1].shape[0], data1['agent_states'][trial_i][:, 1].shape[0])-1, 1]*400, data2['agent_states']
                    [trial_i][:min(data2['agent_states'][trial_i][:, 1].shape[0], data1['agent_states'][trial_i][:, 1].shape[0])-1, 0]*400, c=color_settings['model'], lw=1, ls='-')
...
def plotoverhead_mk(indls, alpha=0.3, **kwargs):
    ...
    if 'ax' in kwargs:
        ...
        with initiate_plot(1.8, 1.8, 300) as fig:
            ...
            ax.plot(np.linspace(0, 230 + 7),
                    np.tan(np.deg2rad(55)) * np.linspace(0, 230 + 7) - 10, c='k', ls=':')
            goal = plt.Circle(np.array(tasks[indls[0]])@np.array([[0, 1], [-1, 0]])
                              * 400, 65, color=color_settings['goal'], alpha=alpha, label='target')
            ax.add_patch(goal)
...
def plotoverhead_simple(states, task, color='b', label='label', ax=None, plotgoal=False):
    if ax:
        for s in states:
            ax.plot(s[:, 0]*200, s[:, 1]*200,
                    color=color, alpha=0.5, label=label)
        if plotgoal:
            goalcircle = plt.Circle(
                (task[0]*200, task[1]*200), 65, edgecolor=color_settings['goal'],  alpha=1, linewidth=2)
            ax.add_patch(goalcircle)
        ax.set_aspect('equal')
        ax.set_xlabel('world x [cm]')
        ax.set_ylabel('world y [cm]')
...
def plotctrl_vs(actions1, actions2, color1='b', color2='r', label1='label1', label2='label2', alpha=1):
    with initiate_plot(3, 3, 300) as fig:
        ax = fig.add_subplot(211)
        totaltime = min(max([len(a) for a in actions1]),
                        max([len(a) for a in actions2]))
...
def intobins(data, nbin=10):
    ...
    counts = np.apply_along_axis(
        lambda a: np.histogram(a, bins=nbin)[0], 0, data)
    return bins, counts
...
def matter_most_eigen_direction():
    ...
    diagnose_plot_theta(agent, env, phi, theta_init,
                        theta_mean+0.1*mattermost.view(-1, 1), 5)
...
def obsvsdensity(noiseparam):
    ...
    if errbar:
        for i in range(4):
            ax.errorbar([i for i in range(4)], y[:, i],
                        err[:, i], label=lables[i])
    else:
        for i in range(4):
    ...
    ax.set_xlabel('density', fontsize=16)
    ax.set_title(
        'inferred observation reliable degree vs ground density', fontsize=20)
...
def plotpert_fill(v, pertv, w, pertw, ax=None, alpha=0.8):
    with initiate_plot(3.8, 1.8, 300) as fig, warnings.catch_warnings():
        ...
        ax.axhline(y=0., color='black', linestyle='--',
                   alpha=alpha*0.5, linewidth=0.5)

        ax.fill_between(
            x, v, pertv, color=color_settings['v'], alpha=alpha, label='foward v')
        ax.fill_between(
            x, w, pertw, color=color_settings['w'], alpha=alpha, label='angular w')
        # legend and label
        handles, labels = ax.get_legend_handles_labels()
        by_label = dict(zip(labels, handles))
...
def plotpertfillwrapper():
    ...
    v, w, pertv, pertw = torch.tensor(v), torch.tensor(
        w), torch.tensor(pertv).float(), torch.tensor(pertw).float()
...
def get_stops(states, tasks, thistask, env, theta, agent, pert=None):
    ...
    for i in indls:
        ...
        if pert is not None:
            env.obs_traj = np.vstack([np.array([0, 0]), np.random.normal(
                0, 1, size=pert.shape)*np.asfarray(theta[4:6]).T + pert, np.zeros(shape=(20, 2))])
        _, _, _, epstates = run_trial(
            agent, env, given_action=None, given_state=None, action_noise=0.1, pert=pert)
    ...
    for i, thetask in enumerate(tasks):
        if stimdur is not None:
            s, a = run_trials(agent, env, phi, theta, thetask, ntrials=ntrials,
                              stimdur=stimdur[i], action_noise=action_noise)
        else:
            s, a = run_trials(agent, env, phi, theta, thetask,
                              ntrials=ntrials, action_noise=action_noise)
        for i in range(ntrials):
            states.append(s[i])
            actions.append(a[i])
    return states, actions
...
def plotpert(v, w, ax=None, alpha=0.8):
    with initiate_plot(3.8, 1.8, 300) as fig, warnings.catch_warnings():
        ...
        ax.plot(x, w, color=color_settings['w'],
                alpha=alpha, label='angular w')
        # legend and label
        handles, labels = ax.get_legend_handles_labels()
        by_label = dict(zip(labels, handles))
        ax.legend(by_label.values(), by_label.keys())
...
def quickoverhead(statelike, ax=None, alpha=0.5):
    with initiate_plot(3.8, 1.8, 300) as fig, warnings.catch_warnings():
        ...
        for key, value in statelike.items():
            ax.plot(torch.cat(value, 1).t()[:, 0], torch.cat(
                value, 1).t()[:, 1], label=key)
        ...
        goal = plt.Circle(
            tasks[ind], 0.13, color=color_settings['goal'], alpha=alpha, label='target')
        ax.add_patch(goal)
        ax.axis('equal')
...
def multimonkeytheta(monkeynames, mus, covs, errs, shifts=None):
    ...
    for i in range(nmonkey):
        if i == 0:
            ax = theta_bar(mus[i], covs[i], label=monkeynames[i], width=1 /
                           (nmonkey+1), shift=shifts[i], err=errs[i], color=colorlist[i])
        else:
            theta_bar(mus[i], covs[i], ax=ax, label=monkeynames[i], width=1 /
                      (nmonkey+1), shift=shifts[i], err=errs[i], color=colorlist[i])
    return ax
...
def multimonkeyobs(monkeynames, mus, covs, errs, ):
    def _bar(finaltheta, xlabels=theta_names[4:6], err=None, ax=None, label=None, shift=0, width=0.5, color=None):
        with initiate_plot(4, 3, 300) as fig, warnings.catch_warnings():
            ...
            if ax is None:
                ax = fig.add_subplot(111)
            # Create bars and choose color
            ax.bar([i+shift for i in range(len(xlabels))], finaltheta,
                   width, yerr=err, label=label, color=color)
            # title and axis names
    ...
    for i in range(nmonkey):
        if i == 0:
            ax = _bar(mus[i][4:6], label=monkeynames[i], width=1/nmonkey,
                      shift=shifts[i], err=errs[i][:, 4:6], color=colorlist[i])
        else:
            _bar(mus[i][4:6], ax=ax, label=monkeynames[i], width=1/nmonkey,
                 shift=shifts[i], err=errs[i][:, 4:6], color=colorlist[i])
    return ax
...
def multimonkeyeig(monkeynames, evectors):
    ...
    for i in range(nmonkey):
        if i == 0:
            ax = theta_bar(evectors[i], label=monkeynames[i],
                           width=1/nmonkey, shift=shifts[i],  color=colorlist[i])
        else:
            theta_bar(evectors[i], ax=ax, label=monkeynames[i],
                      width=1/nmonkey, shift=shifts[i], color=colorlist[i])
    return ax
...
def plotv(indls, resirc, actions=None, **kwargs):
    with initiate_plot(3.8, 1.8, 300) as fig, warnings.catch_warnings():
        ...
        # plot data
        for trial_i in indls:
            ax.plot(actions[trial_i][1:, 0], color=hex2rgb(
                color_settings['a']), alpha=1/len(indls)**.3, label='monkey')

        for trial_i in range(resirc['num_trials']):
            ax.plot(resirc['agent_actions'][trial_i][:, 0], color=hex2rgb(
                color_settings['model']), alpha=1/resirc['num_trials']**.3, label='IRC')
        # legend and label
        ax.spines['top'].set_visible(False)
        ax.spines['right'].set_visible(False)
        ax.set_xlabel('time, dt')
...
def plotw(indls, resirc, actions=None, **kwargs):
    with initiate_plot(3.8, 1.8, 300) as fig, warnings.catch_warnings():
        ...
        # plot data
        for trial_i in indls:
            ax.plot(actions[trial_i][1:, 1], color=hex2rgb(
                color_settings['a']), alpha=1/len(indls)**.3, label='monkey')

        for trial_i in range(resirc['num_trials']):
            ax.plot(resirc['agent_actions'][trial_i][:, 1], color=hex2rgb(
                color_settings['model']), alpha=1/resirc['num_trials']**.3, label='IRC')
        # legend and label
        ax.spines['top'].set_visible(False)
        ax.spines['right'].set_visible(False)
...
def multimonkeyerr(densities, mus, monkeynames):
    def _bar(data, xlabels=densities, err=None, ax=None, label=None, shift=0, width=0.5, color=None, ylabel='radial error [cm]', xlabel='densities'):
        with initiate_plot(4, 4, 300) as fig, warnings.catch_warnings():
            ...
            if ax is None:
                ax = fig.add_subplot(111)
            # Create bars and choose color
            ax.bar([i+shift for i in range(len(densities))], data,
                   width, yerr=err, label=label, color=color)
            # title and axis names
            ax.set_ylabel(ylabel)
    ...
    for i in range(nmonkey):
        if i == 0:
            ax = _bar(mus[i], label=monkeynames[i], width=1 /
                      (nmonkey+0.2), shift=shifts[i], color=colorlist[i])
        else:
            _bar(mus[i], ax=ax, label=monkeynames[i], width=1 /
                 (nmonkey+0.2), shift=shifts[i], color=colorlist[i])
    return ax
...
def plotv_fill(indls, resirc, actions=None, percentile=5, **kwargs):
    ...
    with initiate_plot(3.8, 1.8, 300) as fig, warnings.catch_warnings():
        ...
        ax.fill_between(
            ts, y1=low, y2=high, color=color_settings['a'], alpha=0.5, edgecolor='none', label='monkey')
        ...
        ts = np.arange(len(mk.T))*0.1
        ax.fill_between(
            ts, y1=low, y2=high, color=color_settings['model'], alpha=0.5, edgecolor='none', label='IRC')
...
def process_human_data(subjectls, env):
    ...
    for sub in subjectls:
        ...
        for i in range(ntrial):
            epactions = np.vstack([down_sampling(sub['vel']['v'][i], orignal_dt=0.0012*8) /
                                  worldscale, down_sampling(sub['vel']['w'][i], orignal_dt=0.0012*8)/90]).T
...
def loadmat_(filename, key='subjects'):
    '''
