...
# process the data for irc ----------------------------------------------

def state_step1(px, py, heading, v, w,a, pro_gainv=1, pro_gainw=1,dt=0.006): 
    w=w/180*pi
    ...
    # turn then go
    heading = heading + w*dt
    px=px+np.sin(heading)*v*dt
    py=py+np.cos(heading)*v*dt
...
# test and check mismatch for single trial ----------------------------
...
# target location
# task=exp_data.behav.continuous.y_fly[ind],-exp_data.behav.continuous.x_fly[ind]
task=exp_data.behav.continuous.x_fly[ind],exp_data.behav.continuous.y_fly[ind]

# get the actions
actions=np.array([X['rad_vel'][np.argwhere(trial_idx==ind)],X['ang_vel'][np.argwhere(trial_idx==ind)]])[:,:,0].T

# get the positions
xs=X['x_monk'][np.argwhere(trial_idx==ind)]
ys=X['y_monk'][np.argwhere(trial_idx==ind)]

# run the actions
px, py, heading, v, w = 0,-32.5,0,0,0
log=[]
for a in actions:
    px, py, heading, v, w=state_step1(px, py, heading, v, w, a, dt=0.1)
    log.append([px,py])
log=np.array(log)

plt.plot(log[:,0], log[:,1], label='model')
plt.plot(xs,ys, label='actual')
plt.axis('equal')
plt.scatter(task[0],task[1], label='goal')
plt.legend()
plt.title('mismatch={:.2f} cm'.format(sum(np.power((log[-1] - np.array([xs[-1],ys[-1]]).reshape(-1)),2))**0.5))
plt.show()
...
log=[]
for a in actions:
    px, py, heading, v, w=state_step2(px, py, heading, v, w, a, dt=0.1)
    log.append([px,py])
log=np.array(log)

plt.plot(log[:,0], -log[:,1], label='model')
plt.plot(ys,-xs, label='actual')
plt.axis('equal')
plt.scatter(task[0],task[1], label='goal')
plt.legend()
plt.title('mismatch={:.2f} cm'.format(sum(np.power((log[-1] - np.array([ys[-1],xs[-1]]).reshape(-1)),2))**0.5))


# option 2 rotated for irc -------------------
alltrials=list(set(trial_idx))
ind=alltrials[np.random.randint(0,len(alltrials))]

task=exp_data.behav.continuous.y_fly[ind]+32.5,-exp_data.behav.continuous.x_fly[ind]
...
plt.plot((log[:,0])/500, (-log[:,1])/500, label='model')
plt.plot(ys/500,-xs/500, label='actual')
plt.axis('equal')
plt.scatter(task[0]/500,task[1]/500, label='goal')
plt.scatter((task[0]-32.5)/500,task[1]/500, label='prev goal')
plt.legend()
plt.title('mismatch={:.2f} cm'.format(sum(np.power((log[-1][:2]-np.array([32.5, 0]) - np.array([ys[-1],xs[-1]]).reshape(-1)),2))**0.5))
...
for ind in alltrials:
    ...
    print(len(action),len(states),len(trial_idx[trial_idx==ind]), len(X['y_monk'][trial_idx==ind]))
...
if __name__ == '__main__':
    ...
    # load and test
    datapath="/data/neuraltest/1208pack"
