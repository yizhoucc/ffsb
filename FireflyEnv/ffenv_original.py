...
class FireflyEnv(gym.Env, torch.nn.Module): 
    ...
    def reset(self): # reset env
        ...
        self.stop=False
        min_r = self.goal_radius.item()+self.box*0.2
    ...
    def belief_step(self,  b, ox, a, box): # to belief next
        ...
        B= torch.zeros(5, 5)
        B[-2:, -2:]=torch.diag(torch.Tensor(a))
        # Extended Kalman Filter
        pre_bx_, P = b
        bx_ = self.predict_bx(pre_bx_, a, self.dt, box, self.pro_gains) # prediction
        bx_ = bx_.t() # make a column vector
        _, _, ang, _, _ = torch.split(bx_.view(-1),1)
        A = self.A(pre_bx_,a,self.pro_gains) # calculate the A transition matrix 
        position_uncertainty=torch.diag(self.pro_noise_stds**2) # vw uncertainty
        rotation=torch.zeros(2,2) # rotation matrix turn vw uncertainty into xy
        rotation[0,0]=torch.cos(ang) # here assuming w uncertainty is perpendicular to v
        rotation[0,1]=-torch.sin(ang) # but the w uncertainty is actually an arc, not line
        rotation[1,0]=torch.sin(ang)
        rotation[1,1]=torch.cos(ang)
        position_uncertainty=rotation@(position_uncertainty@(rotation.t()))
        P[:2,:2]=P[:2,:2]+position_uncertainty # xy uncertainty
        # print(P[:2,:2])
        P[2,2]=torch.sqrt(P[2,2])+self.pro_noise_stds[1]**2 # heading angle uncertainty

        P_ = A.mm(P).mm(A.t())+Q # estimate Pt+1 = APA^T+Q, 
        if not is_pos_def(P_): #  debug. 
            print("predicted:", P_)
            print("prev P:", P)
            print("A:", A)
            APA = A.mm(P).mm(A.t())
    ...
    def A(self,x_,a, progain): # create the transition matrix
        '''
        used in kalman filter as transformation matrix for xt to xt+1
        '''
        # retrun A matrix, A*Pt-1*AT+Q to predict Pt
        dt = self.dt
        _, _, ang, _, _ = torch.split(x_.view(-1),1)
        vel=a[0]
        ang_vel=a[1]
        A_ = torch.zeros(5, 5)
        A_[:3, :3] = torch.eye(3)
        A_[0, 2] = - vel*progain[0] * torch.sin(ang+ang_vel*progain[1]*dt) * dt
        A_[1, 2] = vel*progain[0] * torch.cos(ang+ang_vel*progain[1]*dt) * dt
        A_[2,4]=dt
        # A_[3,3]=vel*progain[0]*torch.ones(1)
        # A_[4,4]=ang_vel*progain[1]*torch.ones(1)
        return A_

    def predict_bx(self,x, a, dt, box, pro_gains): # to state next
        '''
        oringal in fireflytask forward, return next state x.
        questions, progain and pronoise is applied even when calculating the new x. is it corret?
        '''
        ...
        vel = 0.0 * vel + pro_gains[0] * a_v # discard prev velocity and new v=gain*new v+noise
        ang_vel = 0.0 * ang_vel + pro_gains[1] * a_w
        ang = ang + ang_vel * dt
        ang = range_angle(ang) # adjusts the range of angle from -pi to pi
