...
class MonkeyDataExtractor():
    def __init__(self, folder_path, task):
        ...
        self.folder_path = folder_path / task
    ...
    def bcm_extract_smr(self):
        ...
        for idx, file_name in enumerate(self.smr_full_file_path):
            ...
            for ch_idx, ch_data in enumerate(seg_reader.analogsignals[:-1]):
                ...
                channel_names.append(ch_data.annotations['stream_id'][0])
    ...
    def bcm_segment(self, lazy_threshold=4000, skip_threshold=400, skip_r_threshold=30, 
        ...
        for session_idx, session_data in enumerate(self.channel_signal_all):
            ...
            for trial_idx in range(end_marker_times.size):
                ...
                trial_data['LDy'] = medfilt(trial_data['LDy'], medfilt_kernel)
                trial_data['LDz'] = medfilt(trial_data['LDz'], medfilt_kernel)
                ...
                eye_ver.append(trial_data['RDz'].values)
                eye_hor.append(trial_data['RDy'].values)
        ...
        self.monkey_trajectory = pd.DataFrame().assign(gain_v=gain_v, gain_w=gain_w, 
                                 ...
                                 relative_angle_end=relative_angle_end, category=category,
                                                      eye_ver=eye_ver, eye_hor=eye_hor)
...
data_path = Path('/data/bruno_normal')
bruno_data_extractor = MonkeyDataExtractor(folder_path=data_path, task='06-13-2017')
bruno_perturb_trajectory = bruno_data_extractor()

newdf = get_gaze_location(bruno_perturb_trajectory)
