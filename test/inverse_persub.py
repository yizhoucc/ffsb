...
for invtag in ['a','h']:
    ...
    for isub in range(0,nsub):
        ...
        trainind_d=set(distsortind[:int(len(distsortind)*3/5)])

        # select left
        trainind_a=set([i if t[1]>0 else False for i,t in enumerate(tasks)])

        # make the training set
        trainind=trainind_d.union(trainind_a)
        print('using {} trials as training set'.format(len(trainind)))
        trainind=list(trainind)
        ...
        if savename.is_file():
            ...
            print('starting new inverse ... save to {}'.format(savename.name))
