...
class SmoothAction1d(gym.Env):
    ...
    def step(self, action, debug={}):
        ...
        if stop or self.trial_timer > self.trial_len:
            ...
            print(self.trial_reward, bool(
                (self.distance()-self.phi[1]) <= 0), self.is_skip())
    ...
    def reset(self,
              ...
              d=None):
        d = torch.zeros(1).uniform_(self.min_distance,
                                    self.max_distance) if not d else d
    ...
    def reset_task_param(self,
        ...
        _pro_gains = torch.zeros(1).uniform_(
        ...
        _dev_action_cost_factor = torch.zeros(1).uniform_(
            self.dev_action_cost_range[0], self.dev_action_cost_range[1])
...
class SmoothAction1dgamma(SmoothAction1d):
    ...
    def __init__(self) -> None:
        ...
        high = np.inf
        self.observation_space = spaces.Box(
            low=low, high=high, shape=(7,), dtype=np.float32)
    ...
    def reset_task_param(self,
        ...
        _dev_action_cost_factor = torch.zeros(1).uniform_(
            self.dev_action_cost_range[0], self.dev_action_cost_range[1])
        _gamma = 1-0.07 * \
            torch.zeros(1).uniform_(self.gamma_range[0], self.gamma_range[1])
        phi = torch.cat([_pro_gains,
                         _goal_radius,
                         _dev_action_cost_factor,
                         _gamma,
    ...
    def reset(self,
        ...
        self.distance = torch.zeros(1).uniform_(
...
class Smooth1dCrossFade(SmoothAction1dRewardRate):
    ...
    def rewardfun(self, action):
        ...
        if stop or self.trial_timer > self.trial_len:  # end of ep
            signal = self.rewardfun_ratio * \
                (reward-cost)+(1-self.rewardfun_ratio)*reward_rate
            done = True
            print(
...
class Simple1d(gym.Env, torch.nn.Module):
    ...
    def __init__(self, arg=None):
        ...
        self.action_space = spaces.Box(
            low=-1., high=1., shape=(1,), dtype=np.float32)
        self.observation_space = spaces.Box(
            low=low, high=high, shape=(1, 14), dtype=np.float32)
    ...
    def reset_task_param(self,
        ...
        _pro_gains = torch.zeros(1).uniform_(
            self.gains_range[0], self.gains_range[1])
        _pro_noise_stds = torch.zeros(1).uniform_(
            self.std_range[0], self.std_range[1])
        _obs_noise_stds = torch.zeros(1).uniform_(
            self.std_range[2], self.std_range[3])
        _goal_radius = torch.zeros(1).uniform_(
            self.goal_radius_range[0], self.goal_radius_range[1])
        _mag_action_cost_factor = torch.zeros(1).uniform_(
            self.mag_action_cost_range[0], self.mag_action_cost_range[1])
        _dev_action_cost_factor = torch.zeros(1).uniform_(
            self.dev_action_cost_range[0], self.dev_action_cost_range[1])
    ...
    def reset_state(self, goal_position=None, initv=None):
        # max d to the goal is world boundary.
        self.goalx = torch.zeros(1).uniform_(
            self.min_distance, self.max_distance)
        ...
        if initv is not None:
            ...
            vctrl = torch.zeros(1).uniform_(
                self.previous_v_range[0], self.previous_v_range[1])
    ...
    def reset_decision_info(self):
        self.decision_info = self.wrap_decision_info(
            b=self.b, time=self.episode_time, task_param=self.theta)
    ...
    def step(self, action, onetrial=False):
        ...
        self.decision_info = self.wrap_decision_info(
            b=self.b, P=self.P, time=self.episode_time, task_param=self.theta)
        # eval
        # if self.no_skip:
        ...
        if end_session:
            print('session reward', self.session_reward, 'percentagge:',
                  self.session_rewarded_trial/self.session_trial_counter)
        if onetrial or self.trial_base:
    ...
    def forward(self, action, task_param, state=None):
        ...
        self.b, self.P = self.belief_step(
            self.b, self.P, self.o, action, task_param=task_param)
        self.decision_info = self.wrap_decision_info(
            b=self.b, P=self.P, time=self.episode_time, task_param=task_param)
        self.episode_time = self.episode_time+1
        end_current_ep = (self.stop or self.episode_time >= self.episode_len)
    ...
    def state_step(self, a, s):
        ...
        w = torch.distributions.Normal(
            0, torch.ones([1, 1])).sample()*self.pro_noise
    ...
    def observations(self, s):
        on = torch.distributions.Normal(
            0, torch.tensor(1.)).sample()*self.theta[2]
...
class FireflyTrue1d_real(gym.Env, torch.nn.Module):
    ...
    def __init__(self, arg=None, kwargs=None):
        ...
        self.action_space = spaces.Box(-np.ones(1),
                                       np.ones(1), dtype=np.float32)
        self.observation_space = spaces.Box(
            low=low, high=high, shape=(1, 15), dtype=np.float32)
    ...
    def reset(self,
        ...
        if new_session:
            if phi is not None:
                ...
                self.phi = self.reset_task_param(
                    pro_gains=pro_gains, pro_noise_stds=pro_noise_stds, obs_gains=obs_gains, obs_noise_stds=obs_noise_stds)
    ...
    def reset_state(self, goal_position=None, initv=None):
        ...
        self.goalx = torch.zeros(1).uniform_(
            self.min_distance, self.max_distance)
        initv_ctrl = torch.zeros(1).uniform_(
            0., 1.) if initv is None else initv
    ...
    def step(self, action, onetrial=False):
        ...
        self.episode_time = self.episode_time+1
        self.sys_vel = self.sys_vel*self.tau_a + \
            action[0]*self.pro_gain*self.tau_b
        ...
        # self.trial_dev+=dev
        if end_current_ep:
            reward_rate = (self.episode_reward -
                           self.trial_sum_cost)/(self.episode_time+5)
            print('reward_rate, ', reward_rate,
    ...
    def if_agent_stop(self, action=None, state=None, sys_vel=None):
        ...
        state = self.s if state is None else state
        px,  vel = torch.split(state.view(-1), 1)
        stop = (abs(vel) <= terminal_vel) and sum(
            self.actions) < self.episode_time
        if stop:
            terminal = True
        if sys_vel is not None:
            stop = (abs(sys_vel) <= terminal_vel)
    ...
    def state_step(self, a, s):
        ...
        vel = self.tau_a * vel + self.tau_b * \
            self.pro_gain * (torch.tensor(1.0)*a[0] + w)
        next_s = torch.cat((px, vel[0])).view(1, -1)
    ...
    def caculate_reward(self):
        if self.stop:
            ...
            # reward = neg_velocity
            reward = 0.
        cost, mag, dev = self.cost_function(
            self.a, self.previous_action, mag_scalar=self.phi[5, 0], dev_scalar=self.phi[6, 0])
    ...
    def observations(self, s):
        ...
        if self.obs_traj is None:
            on = torch.distributions.Normal(
                0, torch.tensor(1.)).sample()*self.obs_noise
        else:
            # *self.obs_noise
            on = self.obs_traj[int(self.episode_time.item())]
        vel = s.view(-1)[-1]  # 1,5 to vector and take last two
        ovel = vel + on
        return ovel.view(-1, 1)
...
class FireflyTrue1d(gym.Env, torch.nn.Module):
    ...
    def reset_decision_info(self):
        self.decision_info = self.wrap_decision_info(
            b=self.b, time=self.episode_timer, task_param=self.theta)
        self.decision_info = row_vector(self.decision_info)
    ...
    def step(self, action, onetrial=False):
        ...
        end_current_ep = (
            action < self.terminal_vel or self.episode_timer >= self.episode_len)
        end_session = (self.session_timer >= self.session_len)
        self.episode_reward, cost, mag, dev = self.caculate_reward()
        self.previous_action = action
        self.trial_sum_cost += cost
        ...
        if end_current_ep:
            reward_rate = (self.episode_reward -
                           self.trial_sum_cost)/(self.episode_timer+5)
        ...
        self.b, self.P = self.belief_step(self.b, self.P, self.o, action)
        self.decision_info = self.wrap_decision_info(
            b=self.b, P=self.P, time=self.episode_timer, task_param=self.theta)
    ...
    def state_step(self, a, s):
        ...
        if self.pro_traj is None:
            w = torch.distributions.Normal(
                0, torch.ones([1, 1])).sample()*self.phi[1]
        else:
            w = self.pro_traj[int(self.episode_time.item())]  # *self.pro_noise
    ...
    def caculate_reward(self):
        ...
        if self.a < self.terminal_vel:
            ...
            if d <= self.phi[3, 0]:
                reward = self.reward
        cost, mag, dev = self.action_cost_wrapper(
            self.a, self.previous_action, mag_scalar=self.phi[4, 0], dev_scalar=self.phi[5, 0])
    ...
    def action_cost_wrapper(self, action, previous_action, mag_scalar, dev_scalar):
        ...
        dev_cost = self.action_cost_dev(
            action, previous_action)*self.cost_scale
        total_cost = mag_scalar*mag_cost+dev_scalar*dev_cost
        return total_cost, mag_cost, dev_cost
...
class Firefly_real_vel(gym.Env, torch.nn.Module):
    ...
    def __init__(self, arg=None, kwargs=None):
        ...
        self.observation_space = spaces.Box(
            low=low, high=high, shape=(1, 39), dtype=np.float32)
        self.cost_function = self.action_cost_wrapper
        self.cost_scale = arg.cost_scale
    ...
    def step(self, action):
        ...
        if self.stop:
            print('distance, ', self.get_distance()[
                  1], 'goal', self.goal_r, 'sysvel', self.sys_vel)
            print('reward: {}, cost: {}, mag{}, dev{}'.format(self.episode_reward -
                  self.trial_sum_cost, self.trial_sum_cost, self.trial_mag, self.trial_dev))
        self.previous_action = action
        return self.decision_info, self.episode_reward-cost, end_current_ep, {}
    ...
    def state_step(self, a, s):
        ...
        if self.pro_traj is None:
            noisev = torch.distributions.Normal(
                0, torch.ones(1)).sample()*self.pro_noisev
            noisew = torch.distributions.Normal(
                0, torch.ones(1)).sample()*self.pro_noisew
        else:
            # *self.pro_noise
            noisev = self.pro_traj[int(self.episode_time.item())]
            # *self.pro_noise
            noisew = self.pro_traj[int(self.episode_time.item())]
    ...
    def caculate_reward(self):
        ...
        cost, mag, dev = self.cost_function(
            self.a, self.previous_action, mag_scalar=self.phi[7, 0], dev_scalar=self.phi[8, 0])
        return reward, self.cost_scale*cost, mag, dev
    ...
    def observations(self, state):
        ...
        if self.obs_traj is None:
            noisev = torch.distributions.Normal(
                0, torch.tensor(1.)).sample()*self.obs_noisev
            noisew = torch.distributions.Normal(
                0, torch.tensor(1.)).sample()*self.obs_noisew
        else:
            # *self.obs_noise
            noise = self.obs_traj[int(self.episode_time.item())]
...
class Firefly_2devcost(Firefly_real_vel):
    ...
    def __init__(self, arg=None, kwargs=None):
        ...
        self.action_space = spaces.Box(
            low=-1., high=1., shape=(2,), dtype=np.float32)
        self.observation_space = spaces.Box(
            low=low, high=high, shape=(1, 40), dtype=np.float32)
        self.cost_function = self.action_cost_wrapper
        self.cost_scale = arg.cost_scale
        self.presist_phi = arg.presist_phi
        self.agent_knows_phi = arg.agent_knows_phi
    ...
    def reset_task_param(self,
        ...
        _dev_action_cost_factor = torch.zeros(1).uniform_(
            self.dev_action_cost_range[0], self.dev_action_cost_range[1])
        _dev_vw_ratio = torch.zeros(1).uniform_(
            self.dev_vw_ratio_range[0], self.dev_vw_ratio_range[1])
...
class FireflySepdev(Firefly_real_vel):
    ...
    def __init__(self, arg=None, kwargs=None):
        ...
        self.action_space = spaces.Box(
            low=-1., high=1., shape=(2,), dtype=np.float32)
        self.observation_space = spaces.Box(
            low=low, high=high, shape=(1, 42), dtype=np.float32)
        self.cost_function = self.action_cost_wrapper
        self.cost_scale = arg.cost_scale
        self.presist_phi = arg.presist_phi
        self.agent_knows_phi = arg.agent_knows_phi
...
class FireflyFinal(Firefly_real_vel):
    ...
    def caculate_reward(self):
        ...
        print('reward', reward, 'dev', dev,
              'actions', self.previous_action, self.a)
        reward = reward
        return reward, self.cost_scale*cost, mag, dev
...
class FireflyFinal2(FireflyFinal):
    ...
    def __init__(self, arg=None, kwargs=None):
        ...
        self.action_space = spaces.Box(
            low=-1., high=1., shape=(2,), dtype=np.float32)
        self.observation_space = spaces.Box(
            low=low, high=high, shape=(1, 27), dtype=np.float32)
        self.no_skip = False
    ...
    def reset_task_param(self,
        ...
        _inital_x_std = torch.zeros(1).uniform_(
            self.inital_cov_range[0], self.inital_cov_range[1])
    ...
    def reset_state(self, goal_position=None, initv=None, initw=None):
        ...
        if initv is not None:
            ...
            v_ctrl = torch.zeros(1).uniform_(
                self.prev_control_range[0], self.prev_control_range[1])
        if initw is not None:
            w_ctrl = initw
        else:
            w_ctrl = torch.zeros(
                1).uniform_(-self.prev_control_range[1]/2, self.prev_control_range[1]/2)
        self.previous_action = torch.tensor([[v_ctrl, w_ctrl]])
        self.s = torch.tensor(
            [[torch.distributions.Normal(0, torch.ones(1)).sample()*0.06*self.theta[10]],
             [torch.distributions.Normal(
                 0, torch.ones(1)).sample()*0.06*self.theta[11]],
             [0.],
             [v_ctrl*self.phi[0]],
             [w_ctrl*self.phi[1]]])
    ...
    def step(self, action):
        ...
        if self.no_skip:
            ...
            end_current_ep = (
                self.stop or self.episode_time >= self.episode_len)
        self.episode_reward, cost, mag, dev = self.caculate_reward(
            action)  # using prev action
        self.trial_sum_cost += cost
        # self.trial_mag_costs.append(mag)
        # self.trial_dev_costs.append(dev)
        # self.trial_actions.append(action)
    ...
    def forward(self, action, task_param, state=None, giving_reward=None):
        ...
        if self.stop and end_current_ep:
            ...
            self.reward_rate = (self.episode_reward -
                                self.trial_sum_cost)/(self.episode_time+5)
            # print('distance, ', self.get_distance()[1], 'goal', self.goal_r,'sysvel',self.sys_vel, 'time', self.episode_time)
            # print('reward: {}, cost: {}, mag{}, dev{}'.format(self.episode_reward-self.trial_sum_cost, self.trial_sum_cost, self.trial_mag, self.trial_dev))
            # return self.decision_info, self.episode_reward-self.trial_sum_cost, end_current_ep, {}
    ...
    def step(self, action, onetrial=False):
        ...
        if end_session:
            ...
            print('reward percent,  ',
                  self.session_reward_trials/self.session_trials)
        return self.decision_info, self.episode_reward-cost, end_session, {}
...
class FireFlyReadySession(FireFlyReady):
    ...
    def step(self, action, next_state=None):
        ...
        if end_current_ep and self.session_timer < self.session_len:
            ...
            # print(self.trial_counter)
            if self.trial_counter != 0 and self.trial_counter % 10 == 0:
                print('rewarded: ', self.recent_rewarded_trials, 'skipped: ',
                      self.recent_skipped_trials, ' out of total: ', self.recent_trials)
                self.recent_rewarded_trials = 0
                self.recent_skipped_trials = 0
                self.recent_trials = 0
            print(
                # 'distance, ', "{:.1f}".format((self.get_distance()[1]-self.goal_r).item()),
                # 'stop',self.stop,
                'reward: {:.1f}, cost: {:.1f}'.format(
                    self.trial_sum_reward, self.trial_sum_cost),
                # 'rate {:.1f}'.format((self.trial_sum_reward-self.trial_sum_cost)/(self.trial_timer+5).item())
            )
...
class FireFlyPaper(FireFlyReady):
    ...
    def wrap_decision_info(self,
        ...
        relative_angle = torch.clamp(relative_angle, -pi, pi)
        # vecL = bcov2vec(P)
        def rotationmatrix(x): return torch.tensor(
